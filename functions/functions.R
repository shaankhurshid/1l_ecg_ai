...
survivor_dt <- function(data,risk_data,time,status,eval.t){
  ...
  for (i in 1:length(unique(data[,get(risk_data)]))){
    km <- survfit(Surv(data[get(risk_data)==unique(data[,get(risk_data)])[order(unique(data[,get(risk_data)]))][i],get(time)],
                       data[get(risk_data)==unique(data[,get(risk_data)])[order(unique(data[,get(risk_data)]))][i],get(status)]) ~ 1)
    est[i] <- 1-stepfun(km$time, c(1, km$surv))(eval.t)
    upper[i] <- 1-stepfun(km$time, c(1, km$lower))(eval.t)
    lower[i] <- 1-stepfun(km$time, c(1, km$upper))(eval.t)
  }
  return(data.frame(est=est,lower=lower,upper=upper))
}
...
auprc <- function(data,time,status,marker,eval.t,tolerance=2){
  cuts = unique(round(as.numeric(unlist(data[,.SD,.SDcols=marker])),tolerance))[order(unique(round(as.numeric(unlist(data[,.SD,.SDcols=marker])),tolerance)))]
  points <- data.table(sens=NULL,ppv=NULL); n<-1
  for (i in cuts){
    time_var <- as.numeric(unlist(data[,.SD,.SDcols=time][,1]))
    delta_var <- as.numeric(unlist(data[,.SD,.SDcols=status][,1]))
    marker_var <- as.numeric(unlist(data[,.SD,.SDcols=marker][,1]))
    sens <- SeSpPPVNPV(cutpoint=i,T=time_var,delta=delta_var,marker=marker_var,cause=1,times=eval.t)$TP[2]
    ppv <- SeSpPPVNPV(cutpoint=i,T=time_var,delta=delta_var,marker=marker_var,cause=1,times=eval.t)$PPV[2]
